...
static void addRawStr(Fmt& fmt, const char* s, size_t len) {
    ...
    ReportDebugIf(fmt.nInst >= dimof(fmt.instructions));
...
static const char* parseArgDefPerc(Fmt& fmt, const char* s) {
    ...
    ReportDebugIf(n > 1); // TODO: only support a single digit for nLen
    if (n > 0) {
        c = *fmtStart++;
        if (c >= '0' && c <= '9') {
            i.width = c - '0';
...
bool Fmt::Eval(const Arg** args, int nArgs) {
    ...
    for (int n = 0; n < nInst; n++) {
        ReportDebugIf(n >= dimof(instructions));

        auto& inst = instructions[n];
        int argNo = inst.argNo;
        ReportDebugIf(argNo >= nArgs);
        if (argNo >= nArgs) {
            isOk = false;
            return false;
        }
...
char* Format(const char* s, const Arg& a1, const Arg& a2, const Arg& a3, const Arg& a4, const Arg& a5, const Arg& a6) {
    ...
    args[nArgs++] = &a5;
    args[nArgs++] = &a6;
    ReportDebugIf(nArgs > dimofi(args));
