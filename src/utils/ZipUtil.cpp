...
bool ZipCreator::AddFileData(const char* nameUtf8, const void* data, size_t size, u32 dosdate) {
    ReportDebugIf(size >= UINT32_MAX);
    ReportDebugIf(str::Len(nameUtf8) >= UINT16_MAX);
    if (size >= UINT32_MAX) {
        return false;
    }
    ...
    ReportDebugIf(local.d.size() != kHdrSize);
    ...
    ReportDebugIf(central.d.size() != kCentralSize);
...
bool ZipCreator::Finish() {
    ReportDebugIf(bytesWritten >= UINT32_MAX);
    ReportDebugIf(fileCount >= UINT16_MAX);
    ...
    ReportDebugIf(eocd.d.size() != kDirSize);
